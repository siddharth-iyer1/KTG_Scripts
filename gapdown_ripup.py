...
class gapdown_ripup(Strategy):
    ...
    def on_start(self, md, order, service, account):
        ...
        service.add_time_trigger(md.market_open_time + service.time_interval(0, 30, 0, 0), repeat_interval=0, timer_id="initial_30min")
    ...
    def on_timer(self, event, md, order, service, account):
        while(event.timer_id != 'initial_30min'):               # During initial 30 min, before flag has been raised
            ...
            order.algo_buy(self.symbol, algorithm='market', intent='init', order_quantity=100)              # Buy first position
            self.buy1 = account[self.symbol].position.captital_long/account[self.symbol].position.shares    # First buy price/share
            break       # Don't want to buy infinite positions
    ...
    def on_trade(self, event, md, order, service, account):
        ...
        if(event.timer_id == 'sell_period_begin'):
            ...
            while event.timer_id != 'market_close':                             # Loops continually until market close
