...
class Gr8Script42def040e6e240a19260bac43bc35730(Strategy):
    ...
    def on_start(self, md, order, service, account):
        ...
        self.entry_data = {}
        self.exit_data = {}
        self.time_ok = 1                                                       # Before 12 PM, ok to trade
        
        service.add_time_trigger(service.time(12, 00))                         # Cover at 3:30 EST
        service.add_time_trigger(service.time(12, 30))
                
    # Alert at 2%
    
    def on_trade(self, event, md, order, service, account):
        if(account[self.symbol].position.shares == 0 and self.qualified >= 0.025 and self.time_ok == 1):                  # Must be new position and trading >2.5% above low
